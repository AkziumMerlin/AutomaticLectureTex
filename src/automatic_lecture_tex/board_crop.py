...
def _dominant_board_roi(
    image: Image.Image, config: VisionConfig
    ...
    # Find the dominant non-bright surface colour. This is intentionally host-side and cheap: it
    # works well for chalk/black boards and fails closed to the uncropped frame when no board-like
    # region is found. A normalized manual ROI remains available for whiteboards/unusual rooms.
    bins = 16
    ...
    score = min(
        1.0,
        0.55 * area_fraction / max(config.board_crop_min_area_fraction, 1e-6) + 0.45 * fill,
    )
...
def detect_board_roi(
    image_path: Path, config: VisionConfig
) -> tuple[tuple[int, int, int, int], float] | None:
    ...
    with Image.open(frame.path) as image:
        ...
        for index, box in enumerate(
            _tile_boxes(
                ...
                overlap_fraction=config.board_crop_tile_overlap,
            )
        ):
